...
import { MAX_APP_STATE_KEY_LEN, MAX_APP_STATE_VALUE_LEN, MAX_APP_STATE_KEYS, APP_STATE_KEY_RE } from '../lib/constants'
...
export class AppStateDO {
  ...
  private async listAll(): Promise<Response> {
    // Cap the number of entries loaded so a large app cannot spike DO memory.
    const map     = await this.state.storage.list<string>({ prefix: AppStateDO.KV_PREFIX, limit: MAX_APP_STATE_KEYS })
    const entries = Array.from(map.entries()).map(([k, value]) => ({
      key: k.slice(AppStateDO.KV_PREFIX.length),
      value,
    }))
    const truncated = map.size >= MAX_APP_STATE_KEYS
    return json(ok({ entries, truncated }))
  }
