...
import { MAX_DOCUMENT_BYTES, GUARD_SCAN_SLICE_BYTES, MAX_VECTOR_CHUNKS, REINDEX_RATE_LIMIT_MAX, RATE_LIMIT_WINDOW_MS, DOCUMENT_UPLOAD_RATE_LIMIT_MAX, DOCUMENT_UPLOAD_RATE_LIMIT_WINDOW_MS } from '../lib/constants'
...
const upload: Handler = async (req, env, params: Params) => {
  ...
  const rl = await rateLimitByIp(req, env, 'rl:doc-upload', DOCUMENT_UPLOAD_RATE_LIMIT_MAX, DOCUMENT_UPLOAD_RATE_LIMIT_WINDOW_MS)
  if (rl) return rl
