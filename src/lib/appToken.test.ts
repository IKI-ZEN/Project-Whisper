...
describe('issueAppToken + verifyAppToken', () => {
  ...
  test('rejects an appId with characters that could break path scoping', async () => {
    await assert.rejects(() => issueAppToken('app/../other', SECRET))
    await assert.rejects(() => issueAppToken('a b', SECRET))
    await assert.rejects(() => issueAppToken('', SECRET))
  })
