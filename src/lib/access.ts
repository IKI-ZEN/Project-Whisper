...
import { JWKS_CACHE_TTL_MS } from './constants'
...
async function fetchPublicKeys(teamDomain: string): Promise<Map<string, CryptoKey>> {
  ...
  keyCache      = map
  keyCacheExpiry = now() + JWKS_CACHE_TTL_MS
  return map
}
...
async function validateJWT(
  ...
  try {
    ...
    // Only RS256 is accepted — reject alg:none and any algorithm confusion attempt
    ...
    // Fetch public key by kid; on miss, invalidate cache and retry once (handles key rotation)
    let keys = await fetchPublicKeys(teamDomain)
    let key  = keys.get(header.kid)
    if (!key) {
      keyCache = null
      keys = await fetchPublicKeys(teamDomain)
