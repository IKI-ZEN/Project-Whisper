...
Cloudflare Access is **optional** — if `CF_ACCESS_AUD` and `CF_ACCESS_TEAM_DOMAIN` are not set, all requests pass through unauthenticated (suitable for development and single-operator deployments).

When the variables are set, every **state-mutation request** (`POST`, `PATCH`, `DELETE`, `PUT`) under `/api/` is gated. The Worker:
...
|----------|----------|-------------|
| `CF_ACCESS_AUD` | If using Access | Audience tag from the Access application settings (a 64-char hex string). |
| `CF_ACCESS_TEAM_DOMAIN` | If using Access | Your team domain, e.g. `yourteam.cloudflareaccess.com`. |

Set both in `.dev.vars` for local testing or in `wrangler.toml` secrets for production. When either is absent the `requireAccess` function short-circuits with `{ deny: null, identity: null }`.
...
3. Under **Include**, add an **Email domain** rule with your team's domain (e.g., `iki-zen.com.au`).
