...
- **Dashboard generation unified with App Builder** — `mode='dashboard'` removed from `POST /api/vibes`. Dashboards now go through the App Builder WebSocket pipeline (`POST /api/v2/build`), producing multi-file apps at `/build/:id` with no 50 KB limit, Cloudflare Pages deploy support, and `platformApiHint` injected when description matches dashboard/metric/monitor keywords. The hint teaches the AI to pull live platform data from `GET /api/app/__BUILD_ID__/platform/{apps,environments,labs,builds,metrics,events,usage,probes}` using the app token injected server-side. `VibeMode` narrows to `'app' | 'environment'`; the Vibe Coder UI routes dashboard mode to `doAppBuild()` (App Builder flow) and shows the streaming build panel.

- **Vision re-centring — Lab, Environments, and Whisperer integration**
  - **Lab** (formerly Environment multi-model comparison): comparison workspaces moved from `/api/environments/*` → `/api/lab/*`, URLs `/lab/:id`, KV flag renamed `fromEnv → fromLab`. `environments.html` is repurposed as the Lab Builder; `/lab.html` redirects to it. Full lifecycle (create, patch, fork, export/import) preserved at new routes.
  - **Environments** now mean agentic specialised workspaces — domain-expert AI assistants generated by `POST /api/vibes` with `mode='environment'`. Served at `/env/:id` as a purpose-built workspace with a chat panel and a Whisperer analysis panel. Registered with `fromVibe: true` and `fromEnv: true` in KV.
  - **Whisperer integration for environments** — `SandboxConfig` gains `whispererFeatures?: string[]`. The Vibe coder in environment mode auto-selects 2–4 domain-relevant analysis features from the 11-tool Whisperer suite. The `/env/:id` workspace surfaces these as action buttons in a right-hand panel; buttons enable after the first model response and post to the relevant Whisperer endpoint, showing a one-line summary inline.
  - **Vibe coder gains four modes**: Quick Sandbox (`mode='app'`, default), Environment (`mode='environment'`), Dashboard (`mode='dashboard'`), App Builder (v2 WebSocket). `POST /api/vibes` now accepts `mode` and returns `envUrl: /env/:id` for environment mode.
  - **Sandbox filter** gains `?only=labs` (returns `fromLab: true` entries) and `?only=dashboards`. `?only=apps` now excludes Labs, Environments, and Dashboards. `?only=envs` returns only the new agentic environments.
  - **Dashboard** (`/dashboard`) gains separate stat cards for Environments and Labs.
  - **Platform proxy** (`GET /api/app/:id/platform/*`) gains a `labs` resource alongside the existing `apps`, `environments`, `builds`, `metrics`, `events`, `usage`, and `probes` resources.
