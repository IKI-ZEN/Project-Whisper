...
An Environment is a domain-expert AI workspace generated by the Vibe coder with `mode='environment'`. You describe a field ("a penetration-testing assistant", "a sales intelligence workspace") and the platform generates a tailored system prompt, picks appropriate Whisperer analysis features for the domain, and launches it at `/env/:id`. The environment workspace has a chat panel on the left and a Whisperer analysis panel on the right — analysis buttons activate after the first response and let you run tools like sensitivity analysis, PII scanning, or guard probing against the last reply. Environments are stored as `SandboxDO` instances with `fromEnv: true` in KV metadata.

### Q: What is the difference between a Sandbox and an Environment?

Both are `SandboxDO` instances. A Sandbox is a general-purpose AI assistant. An Environment is a domain-expert workspace auto-configured by the Vibe coder — it has a tailored system prompt, auto-selected Whisperer analysis features (`whispererFeatures[]`), and a two-panel UI (chat + Whisperer panel). Environments are created via `POST /api/vibes` with `mode='environment'`; all other sandbox routes (run, stream, fork, export, history) apply unchanged. If you want multi-model comparison, create a **Lab** via the Lab Builder (`/environments.html`) — Labs fan a prompt to up to 4 models simultaneously and live at `/lab/:id`.
...
**Labs** (`/lab/:id`) provide multi-model comparison. When a Lab has multiple models in its `envModels` array (up to 4), every message fans out to all configured models simultaneously. Responses stream in parallel into side-by-side columns. Compare mode is triggered automatically whenever `envModels.length > 1`. Create a Lab at `/environments.html` (the Lab Builder).
...
When `SIGNING_SECRET` is set (generate one with `openssl rand -hex 32`), `GET /api/sandbox/:id/export` and `GET /api/lab/:id/export` include an HMAC-SHA256 signature field in the returned JSON. On import (`POST /api/sandbox/import` or `POST /api/lab/import`), the platform verifies the signature and rejects any config that has been modified in transit with a 422. This provides cryptographic provenance guarantees when sharing sandbox or environment configurations between separate deployments.
